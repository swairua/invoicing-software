  ...
  try {
    ...
    for (const quotation of quotations) {
      ...
      if (existingItems[0].count > 0) {
        console.log(`⏭️ Quotation ${quotation.quote_number} already has items, skipping`);
        continue;
      }
      ...
      for (let i = 0; i < selectedProducts.length; i++) {
        ...
        console.log(`   ✅ Added: ${quantity}x ${product.name} @ ${unitPrice} = ${lineTotal.toFixed(2)}`);
      }
      ...
      console.log(`   💰 Updated totals: Subtotal: ${subtotal.toFixed(2)}, VAT: ${quotationVatTotal.toFixed(2)}, Total: ${quotationTotal.toFixed(2)}`);
    }
