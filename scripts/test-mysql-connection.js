...
async function testConnection() {
  try {
    ...
    if (connected) {
      ...
      // Test a simple query
      const result = await Database.query('SELECT COUNT(*) as company_count FROM companies');
      console.log(`📊 Companies in database: ${result.rows[0].company_count}`);
      
      // Test product query
      const products = await Database.query('SELECT COUNT(*) as product_count FROM products');
