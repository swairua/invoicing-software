...
async function createSampleData() {
  ...
  try {
    ...
    if (customers.length === 0) {
      console.log("❌ No customers found. Please ensure customers exist first.");
      return;
    }

    console.log(`📋 Found ${customers.length} customers to create invoices for`);
    ...
    for (const invoice of sampleInvoices) {
      try {
        ...
        `, [
          companyId, invoice.customer_id, invoice.invoice_number, invoice.status,
          ...
          invoice.total_amount, userId
        ]);
        console.log(`  ✅ Created ${invoice.invoice_number} (${invoice.status}) - KES ${invoice.total_amount}`);
      } catch (error) {
        console.log(`  ❌ Failed to create ${invoice.invoice_number}: ${error.message}`);
      }
    }
    ...
    for (const payment of samplePayments) {
      try {
        ...
        `, [
          companyId, customers[0].id, payment.amount, payment.payment_method,
          payment.payment_date, userId
        ]);
        console.log(`  ✅ Created payment of KES ${payment.amount} via ${payment.payment_method}`);
      } catch (error) {
    ...
    console.log(`Total Revenue (paid invoices): KES ${totalRevenue[0].totalRevenue}`);
    ...
    console.log(`Outstanding Invoices (sent/overdue): KES ${outstanding[0].outstandingInvoices}`);
    ...
    console.log(`Recent Payments (30 days): KES ${recentPayments[0].recentPayments}`);
