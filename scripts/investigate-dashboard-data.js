  ...
  try {
    ...
    if (tableNames.includes("invoices")) {
      ...
      if (invoices.length > 0) {
        invoices.forEach((inv) => {
          console.log(
            `  • ${inv.invoice_number || inv.id}: status='${inv.status}', amount=${inv.total_amount}, date=${inv.created_at}`,
          );
        });
        ...
        statusCounts.forEach((stat) => {
          console.log(
            `  • ${stat.status}: ${stat.count} invoices, total: ${stat.total}`,
          );
        });
      } else {
    ...
    if (tableNames.includes("payments")) {
      ...
      if (payments.length > 0) {
        payments.forEach((pay) => {
          console.log(
            `  • Amount: ${pay.amount}, method: ${pay.payment_method || "N/A"}, date: ${pay.created_at}`,
          );
        });
      } else {
    ...
    if (tableNames.includes("invoices")) {
      try {
        ...
        console.log(
          `Total Revenue (paid invoices): ${totalRevenue[0].totalRevenue}`,
        );
        ...
        console.log(
          `Outstanding Invoices (sent/overdue): ${outstanding[0].outstandingInvoices}`,
        );
      } catch (error) {
    ...
    if (tableNames.includes("payments")) {
      try {
        ...
        console.log(
          `Recent Payments (30 days): ${recentPayments[0].recentPayments}`,
        );
      } catch (error) {
