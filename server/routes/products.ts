...
router.get("/", async (req, res) => {
  ...
  try {
    ...
    // If no products found and we're in mock mode, return sample data
    if (result.products.length === 0 && result.total === 0) {
      console.log("📋 No products found, returning mock sample data");
      const mockProducts = [
        {
          id: "mock-1",
      ...
      console.log("📤 Sending mock products response");
      res.json(response);
      return;
    }
