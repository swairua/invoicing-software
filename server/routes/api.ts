  ...
  try {
    ...
    for (const proforma of result.rows) {
      try {
        ...
        console.warn(
          `Failed to fetch items for proforma ${proforma.id}:`,
          itemError,
        );
        proforma.items = [];
      }
    ...
    console.log("📦 Using fallback proforma list data with line items");
        ...
        items: [
          {
            ...
            product_description:
              "High-quality disposable surgical masks - Box of 50",
            quantity: 100,
            unit_price: 150.0,
