...
export default function TestUpdate() {
  ...
  const testUpdateProduct = async () => {
    try {
      ...
      if (response.ok) {
        setResult(`✅ SUCCESS! Product updated successfully!\n\nResponse: ${JSON.stringify(data, null, 2)}`);
  ...
  const testOriginalEndpoint = async () => {
    try {
      ...
      if (response.ok) {
        setResult(`✅ Original endpoint worked: ${JSON.stringify(data, null, 2)}`);
      } else {
        setResult(`❌ Original endpoint failed (expected): ${response.status}\nError: ${data.error || 'Unknown error'}`);
      }
    } catch (error) {
    ...
    <div className="container mx-auto py-6">
      ...
      <Card className="mb-6">
        ...
        <CardContent className="space-y-4">
          ...
          {result && (
            <div className={`p-3 rounded whitespace-pre-wrap font-mono text-sm ${
              result.startsWith('✅') 
          ...
          <div className="text-sm text-gray-600">
            <p><strong>Safe Update:</strong> Uses the new test endpoint with forced null category</p>
            <p><strong>Original Endpoint:</strong> Uses the problematic original endpoint (should fail)</p>
          </div>
        </CardContent>
      </Card>
    </div>
