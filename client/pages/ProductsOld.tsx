...
const mockProducts: Product[] = [
  ...
  },
  {
    ...
    notes:
      "High-value item. Requires assembly. Popular among corporate clients.",
    isActive: true,
    status: "active",
    companyId: "1",
...
export default function Products() {
  ...
  const getStockStatus = (product: Product) => {
    ...
    if (product.currentStock >= product.maxStock)
      return { status: "Overstock", color: "secondary" };
  ...
  const lowStockProducts = products.filter(
    (p) => p.currentStock <= (p.reorderLevel || p.minStock),
  ).length;
                      ...
                      <div className="space-y-2">
                        ...
                        >
                          ...
                          <SelectContent className="max-h-[200px]">
                            ...
                            {UnitConverter.getCommonUnits().weight.map(
                              (unit) => (
                                <SelectItem key={unit.id} value={unit.id}>
                                  {unit.name} ({unit.symbol})
                                </SelectItem>
                            ...
                            {UnitConverter.getCommonUnits().volume.map(
                              (unit) => (
                                <SelectItem key={unit.id} value={unit.id}>
                                  {unit.name} ({unit.symbol})
                                </SelectItem>
                            ...
                            {UnitConverter.getCommonUnits().length.map(
                              (unit) => (
                                <SelectItem key={unit.id} value={unit.id}>
                                  {unit.name} ({unit.symbol})
                                </SelectItem>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">
              <span className="text-success font-medium">{activeProducts}</span>{" "}
              active
            </p>
          </CardContent>
        </Card>
