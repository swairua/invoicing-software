...
export default function QuickActions() {
  ...
  React.useEffect(() => {
    const loadData = async () => {
      try {
        const [customersData, productsData, invoicesData, quotationsData] =
          await Promise.all([
  ...
  const handleCreateDocument = async (documentType: string) => {
    ...
    if (hasInvalidProducts) {
      toast({
        title: "Validation Error",
        description:
          "Please ensure all products are selected with valid quantities.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      const customer =
        customers && customers.find((c) => c.id === selectedCustomer);
  ...
  const renderDocumentForm = (documentType: string) => {
    ...
    const isFromQuotation =
      isDeliveryNote || documentType === "Proforma Invoice";
      ...
      <div className="space-y-6 pb-4">
            ...
            <div className="bg-muted/20 rounded-lg p-4 space-y-2">
              {(() => {
                const subtotal = selectedProducts.reduce((sum, item) => {
                  ...
                  return (
                    sum + (product ? product.sellingPrice * item.quantity : 0)
                  );
                }, 0);
                const vatAmount = subtotal * 0.16;
                const total = subtotal + vatAmount;
        ...
        <div className="flex flex-col sm:flex-row justify-end gap-3 pt-6 border-t">
          ...
          <Button
            ...
            disabled={
              isLoading || !selectedCustomer || selectedProducts.length === 0
            }
            className="w-full sm:w-auto"
          >
            Save as Draft
                ...
                <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
                  {actions.map((action) => (
                    <div key={action.title}>
                      {action.dialog ? (
                        <Dialog
                          open={openDialog === action.title}
                          onOpenChange={(open) =>
                            setOpenDialog(open ? action.title : null)
                          }
                        >
